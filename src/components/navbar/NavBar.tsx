...
interface ILink {
  link: { id: number; title: string; icon: JSX.Element };
}

function NavBar() {
  return (
    <nav className="col-span-2 border-r border-gray-200 min-h-[90vh] w-[80px] xl:w-[250px] pt-6 px-1 flex flex-col items-start justify-between">
      <div className="space-y-8 w-full ">
        {navLinks.slice(0, 9).map((link) => (
          <NavItem link={link} key={link.id} />
        ))}
        <div className="w-full border-t border-gray-200" />
        {navLinks.slice(9, 10).map((link) => (
          <NavItem link={link} key={link.id} />
        ))}
      </div>
    </nav>
